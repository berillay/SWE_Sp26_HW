...
class MainTestCase(unittest.TestCase):
    ...
    def test_calculate_success(self, mock_get):
        ...
        # FARKLI BİLGİSAYARLARDA küsurat farkından dolayı testin patlamaması için 
        # tam sayıları aramak yerine doğrudan BAŞARI MESAJINI arıyoruz.
        html_data = response.get_data(as_text=True)
        self.assertTrue("Satellite/space object position and velocity calculated successfully" in html_data)
        self.assertTrue("Position (X, Y, Z) in km:" in html_data)

    # 4. API'den geçersiz (None) yanıt gelmesi durumunu test eder
